...
class dataset_dict_generator():
    ...
    def __init__(self, lidar_dataset_path='', pose_dataset_path='',
        ...
        for sequence_num in np.array(sequence_to_use):
            ...
            ### LiDAR data path accumulation ###
            lidar_base_path = self.lidar_dataset_path + '/' + sequence_num + '/velodyne'
            lidar_data_name = np.array(sorted(os.listdir(lidar_base_path)))
            ...
            for label in np.unique(cluster_labels):
                ...
                if len(label_idx[0]) > 1:
                    ...
                    base_path_list = np.array([lidar_base_path] * len(train_idx))
                    path_list_append = np.array(['/'] * len(train_idx))
                    combined_path_list = np.core.defchararray.add(base_path_list, path_list_append)
                    combined_path_list = np.core.defchararray.add(combined_path_list, lidar_data_name[train_idx])
                    ...
                    ### Create positive image path list for each cluster label ###
                    base_path_list = np.array([lidar_base_path] * len(label_idx))
                    path_list_append = np.array(['/'] * len(label_idx))
                    combined_path_list = np.core.defchararray.add(base_path_list, path_list_append)
                    combined_path_list = np.core.defchararray.add(combined_path_list, lidar_data_name[label_idx])
                    self.seq_path_dict['{}-{}'.format(sequence_num, label)] = combined_path_list
            ...
            for img_name, pose, cluster_label, data_type in zip(lidar_data_name, pose_list, cluster_labels, data_type_list):
                
                data = [self.data_idx, sequence_num, self.sequence_idx, \
                        lidar_base_path + '/' + img_name, \
...
class sensor_dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, lidar_dataset_path='', pose_dataset_path='',
        ...
        self.dataset_dict_generator = dataset_dict_generator(lidar_dataset_path=lidar_dataset_path, pose_dataset_path=pose_dataset_path,
    ...
    def __getitem__(self, index):
        ...
        anchor_img = self.lidar_range_img_generator.convert_range_img(pcd_path=item[3], output_type='img_pixel')
        anchor_img = np.squeeze(anchor_img, axis=0)
        anchor_img = cv.resize(anchor_img, dsize=(self.output_resolution[0], self.output_resolution[1]), interpolation=cv.INTER_CUBIC)
        
        anchor_img = Image.fromarray(anchor_img.astype(np.uint8)).convert('RGB')
        ...
        if len(positive_path_list) <= 1:
            ...
            positive_img = self.lidar_range_img_generator.convert_range_img(item[3], output_type='img_pixel')
            positive_img = np.squeeze(positive_img, axis=0)
            positive_img = Image.fromarray(positive_img.astype(np.uint8)).convert('RGB')
            ...
            positive_img = self.lidar_range_img_generator.convert_range_img(positive_img_path, output_type='img_pixel')
            positive_img = np.squeeze(positive_img, axis=0)
        ...
        negative_img = self.lidar_range_img_generator.convert_range_img(pcd_path=negative_img_path, output_type='img_pixel')
        negative_img = np.squeeze(negative_img, axis=0)
        negative_img = cv.resize(negative_img, dsize=(self.output_resolution[0], self.output_resolution[1]), interpolation=cv.INTER_CUBIC)
        
        negative_img = Image.fromarray(negative_img.astype(np.uint8)).convert('RGB')
