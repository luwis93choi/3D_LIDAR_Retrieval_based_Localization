...
class sensor_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        current_img = np.array(Image.open(item[2]))
        current_img = cv.cvtColor(current_img, cv.COLOR_RGB2BGR)
        current_img = cv.resize(current_img, dsize=(self.output_resolution[0], self.output_resolution[1]), interpolation=cv.INTER_CUBIC)
        current_img = TF.to_tensor(current_img)

        # pose_6DOF = [float(i) for i in item[4:]]
        pose_6DOF = [float(i) for i in item[3:]]
        pose_6DOF = torch.tensor(pose_6DOF)

        # return lidar_range_img, current_img, pose_6DOF
        return current_img, pose_6DOF
