...
for lidar_data, ref_img_data in zip(lidar_files, img_files):
    ...
    resized_range_img = cv.resize(range_img, dsize=(1280, 240), interpolation=cv.INTER_CUBIC)
    ...
    resized_ref_img = cv.resize(ref_img, dsize=(1280, 240), interpolation=cv.INTER_CUBIC)
